...
func (h *Socks5Handler) handleSocks5(conn net.Conn) error {
	...
	if request[0] != socks5Version {
		h.sendSocks5Reply(conn, generalFailure, nil)
		return errors.New("invalid SOCKS5 request version")
	}

	command := request[1]
	if command != connectCommand {
		h.sendSocks5Reply(conn, commandNotSupported, nil)
	...
	case domainAddress:
		...
		port = int(binary.BigEndian.Uint16(portBytes))
	case ipv6Address:
		h.sendSocks5Reply(conn, addressTypeNotSupported, nil)
	...
	h.sendSocks5Reply(conn, succeeded, localAddr)
...
func (h *Socks5Handler) authenticate(reader *bufio.Reader, conn net.Conn) error {
	...
	zap.L().Info("SOCKS5 authentication succeeded", zap.String("username", username))
	return nil
}
...
func (h *Socks5Handler) sendSocks5Reply(conn net.Conn, rep byte, bindAddr *net.TCPAddr) {
	...
	if bindAddr != nil {
		if ipv4 := bindAddr.IP.To4(); ipv4 != nil {
			response = append(response, ipv4Address)
			response = append(response, ipv4...)
		} else {
			response = append(response, ipv6Address)
